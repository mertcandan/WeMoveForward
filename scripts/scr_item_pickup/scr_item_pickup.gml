...
if (argument0 == item.brokenbonecomb){
	...
	text = [[],[],[],[],[["A tiny rib? Oh, it’s a comb. Talk about leaning hard into a sea aesthetic…."]]];
	speakers = [];
	character_turns = [[],[],[],[],[[0]]];
	dialogue_index = 0;
	var tbox = create_textbox(text, speakers, character_turns, dialogue_index);
...
if (argument0 == item.dirtyribbon){
	global.metadialoguestep = 0
	text = [[],[],[],[],[["You found a dirty ribbon! …….You don’t know why you’re carrying straight up garbage…. but it just feels wrong to throw it away for some reason."]]];
	speakers = [];
	character_turns = [[],[],[],[],[[0]]];
	dialogue_index = 0;
	var tbox = create_textbox(text, speakers, character_turns, dialogue_index);
...
if (argument0 == item.heartshapedglasses){
	global.metadialoguestep = 0
	text = [[],[],[],[],[["You found heart-shaped glasses! Trendy! These are definitely a child’s but they still fit onto your face. Barely. You start feel a migraine going on."]]];
	speakers = [];
	character_turns = [[],[],[],[],[[0]]];
	dialogue_index = 0;
	var tbox = create_textbox(text, speakers, character_turns, dialogue_index);
...
if (argument0 == item.coinonastring){
	global.metadialoguestep = 0
	text = [[],[],[],[],[["You found a coin on a string! As you hold it up, its slowly turns, reflecting light into your eyes. This makes the beautiful woman carved into it look like she’s winking at you! You feel like you should pray to her for some reason."]]];
	speakers = [];
	character_turns = [[],[],[],[],[[0]]];
	dialogue_index = 0;
	var tbox = create_textbox(text, speakers, character_turns, dialogue_index);
